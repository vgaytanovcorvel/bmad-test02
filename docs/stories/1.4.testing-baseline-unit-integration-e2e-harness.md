...
- [ ] **Task 1: Configure Jest for Unit & Integration Test Separation** (AC: 1)
  - [ ] Update Jest configuration files to distinguish unit vs integration test patterns
  - [ ] Set up separate naming conventions: `*.spec.ts` for unit, `*.integration.spec.ts` for integration
  - [ ] Configure test execution scripts to run unit and integration tests separately
  - [ ] Verify coverage thresholds apply to combined unit+integration results
  - [ ] Create sample unit tests for each workspace project (`libs/engine`, `libs/shared`, `apps/ui`)

- [ ] **Task 2: Create Integration Test Harness for Game Flow** (AC: 2)
  - [ ] Set up @testing-library/angular integration testing environment
  - [ ] Create integration test exercising complete move placement workflow
  - [ ] Test human move placement and computer response integration
  - [ ] Verify game state transitions through integration testing
  - [ ] Include game service and component interaction testing
  - [ ] Ensure integration tests use realistic game scenarios

- [ ] **Task 3: Enhance Playwright E2E Configuration** (AC: 3)
  - [ ] Verify existing Playwright configuration covers all browsers
  - [ ] Expand existing health check E2E test for comprehensive validation
  - [ ] Add E2E testing for main application routes and navigation
  - [ ] Configure proper test data setup and teardown for E2E tests
  - [ ] Include accessibility testing in E2E test suite

- [ ] **Task 4: Create Testing Scripts and Documentation** (AC: 4)  
  - [ ] Add separate package scripts for `test:unit`, `test:integration`, `test:e2e`
  - [ ] Update existing `test` script to run unit+integration (maintaining current behavior)
  - [ ] Document testing execution order in CONTRIBUTING.md
  - [ ] Create testing guidelines for each test type (unit/integration/e2e)
  - [ ] Document when to use each testing approach
  - [ ] Include debugging and troubleshooting guides for testing
