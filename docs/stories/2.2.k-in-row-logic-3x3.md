...
- [ ] **Task 1: Implement Row-based Win Detection** (AC: 1)
  - [ ] Create `checkRows(board: Cell[]): number[][]` method for 3x3 board
  - [ ] Iterate through rows 0-2, checking positions [0,1,2], [3,4,5], [6,7,8]
  - [ ] Return array of winning row coordinates when k=3 consecutive cells match
  - [ ] Optimize with early exit when no player occupies sufficient cells
  - [ ] Unit test all row win scenarios and edge cases

- [ ] **Task 2: Implement Column-based Win Detection** (AC: 1)
  - [ ] Create `checkColumns(board: Cell[]): number[][]` method for 3x3 board
  - [ ] Iterate through columns 0-2, checking positions [0,3,6], [1,4,7], [2,5,8]
  - [ ] Return array of winning column coordinates when k=3 consecutive cells match
  - [ ] Ensure consistent coordinate format with row detection
  - [ ] Unit test all column win scenarios and boundary conditions

- [ ] **Task 3: Implement Diagonal Win Detection** (AC: 1)
  - [ ] Create `checkDiagonals(board: Cell[]): number[][]` method for 3x3 board
  - [ ] Check main diagonal positions [0,4,8] for k=3 consecutive matches
  - [ ] Check anti-diagonal positions [2,4,6] for k=3 consecutive matches
  - [ ] Return array of winning diagonal coordinates when matches found
  - [ ] Unit test both diagonal win scenarios and mixed cases

- [ ] **Task 4: Integrate kInRow Method with Engine Interface** (AC: 2)
  - [ ] Implement `kInRow(state: GameState): number[][]` method in engine
  - [ ] Combine results from row, column, and diagonal detection methods
  - [ ] Return all winning line coordinates when terminal win state occurs
  - [ ] Ensure method works with existing `GameState` interface from Story 2.1
  - [ ] Handle multiple simultaneous wins (return all winning lines)

- [ ] **Task 5: Implement Draw Detection Logic** (AC: 3)
  - [ ] Create `isDraw(state: GameState): boolean` method
  - [ ] Check if all board positions are occupied (no null cells)
  - [ ] Verify no winning conditions exist using kInRow method
  - [ ] Integrate with `isTerminal(state)` method for complete game end detection
  - [ ] Unit test draw scenarios with full boards and no winners

- [ ] **Task 6: Optimize Win Detection Performance** (AC: 1)
  - [ ] Avoid unnecessary object allocations in detection loops
  - [ ] Use simple iteration patterns instead of complex array methods
  - [ ] Cache intermediate results where beneficial without memory overhead
  - [ ] Ensure detection completes in <1ms for 3x3 boards
  - [ ] Performance test with multiple consecutive win detections

- [ ] **Task 7: Create Comprehensive Unit Test Suite** (AC: 4)
  - [ ] Test all 8 possible winning lines: 3 rows, 3 columns, 2 diagonals
  - [ ] Test single win scenarios for each line type
  - [ ] Test multiple simultaneous win scenarios (corner cases)
  - [ ] Test near-miss scenarios (2 in a row but no win)
  - [ ] Test draw scenarios with full board and no wins
  - [ ] Test edge cases with empty boards and partial fills
  - [ ] Achieve 100% coverage on all win detection methods
