...
- [ ] **Task 1: Implement Legal Moves Detection** (AC: 1)
  - [ ] Create `legalMoves(state: GameState): number[]` method in Engine interface
  - [ ] Return array of positions where board cells are null (empty)
  - [ ] Handle both 3x3 and 4x4 board sizes dynamically based on GameState config
  - [ ] Ensure method works with existing GameState interface and board array
  - [ ] Return empty array when game is in terminal state (won/draw)
  - [ ] Unit test all board configurations and game states

- [ ] **Task 2: Implement Move Application with Player Alternation** (AC: 2)
  - [ ] Create `applyMove(state: GameState, move: Move): GameState` method in Engine interface
  - [ ] Validate move position is within legal moves before application
  - [ ] Apply move to board array at specified position with current player
  - [ ] Alternate currentPlayer from X to O or O to X after move application
  - [ ] Add move to moveHistory array with player, position, and timestamp
  - [ ] Return completely new immutable GameState object (never mutate input)
  - [ ] Update game status using existing terminal detection logic

- [ ] **Task 3: Terminal State Move Validation** (AC: 3)
  - [ ] Add validation to prevent moves when game status is 'won' or 'draw'
  - [ ] Throw descriptive GameError with clear message when attempting invalid moves
  - [ ] Include current game status and attempted move position in error messages
  - [ ] Ensure error handling integrates with existing Engine interface patterns
  - [ ] Unit test error conditions and error message accuracy

- [ ] **Task 4: Immutability and State Consistency** (AC: 2, 4)
  - [ ] Ensure all state transitions create new objects without mutating originals
  - [ ] Verify board array, moveHistory, and all nested objects are immutable
  - [ ] Update startTime/endTime appropriately during state transitions
  - [ ] Maintain config object consistency across state transitions
  - [ ] Integration tests verify original state objects remain unchanged
  - [ ] Performance tests ensure efficient object creation without memory leaks

- [ ] **Task 5: Cross-Board Size Compatibility** (AC: 1, 2)
  - [ ] Ensure legalMoves works correctly for both 3x3 (9 positions) and 4x4 (16 positions)
  - [ ] Validate applyMove handles board size differences in position bounds
  - [ ] Integration tests with various board configurations and game progressions
  - [ ] Ensure move validation uses config.boardSize for position bounds checking
  - [ ] Test edge cases with different board sizes and game states

- [ ] **Task 6: Comprehensive Unit Test Suite** (AC: 4)
  - [ ] Test legalMoves with empty, partial, and full boards for both sizes
  - [ ] Test applyMove with valid moves, invalid positions, and occupied cells
  - [ ] Test player alternation correctness through multiple move sequences
  - [ ] Test immutability by verifying original state objects are unchanged
  - [ ] Test terminal state move rejection with descriptive error messages
  - [ ] Test move history tracking and timestamp accuracy
  - [ ] Achieve 100% coverage on all move validation and application methods
