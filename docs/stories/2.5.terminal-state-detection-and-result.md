...
- [ ] **Task 1: Implement Terminal State Detection Logic** (AC: 1)
  - [ ] Create `isTerminal(state: GameState): boolean` method in Engine interface
  - [ ] Detect win conditions using existing `kInRow()` method integration
  - [ ] Detect draw conditions when board is full and no winner exists
  - [ ] Handle both 3x3 and 4x4 board sizes with dynamic board evaluation
  - [ ] Ensure method returns false for ongoing games with remaining moves
  - [ ] Unit test all terminal and non-terminal game scenarios

- [ ] **Task 2: Implement Winner Determination Logic** (AC: 1)
  - [ ] Create `winner(state: GameState): Player | null` method in Engine interface
  - [ ] Return winning player when terminal state has a winner
  - [ ] Return null for draw games (terminal but no winner)
  - [ ] Integrate with existing win detection logic from kInRow method
  - [ ] Handle edge cases with multiple winning lines (return first winner)
  - [ ] Add proper error handling for non-terminal state queries

- [ ] **Task 3: Complete Terminal State Information** (AC: 2)
  - [ ] Ensure GameState includes complete terminal information
  - [ ] Populate winningLine field with winning coordinates when game ends
  - [ ] Track final board state accurately in terminal conditions
  - [ ] Include last move information in move history
  - [ ] Set endTime timestamp when terminal state is reached
  - [ ] Verify all terminal state data is immutable and complete

- [ ] **Task 4: Cross-Board Size Terminal Detection** (AC: 3)  
  - [ ] Implement terminal detection for 3x3 boards (9 cells, k=3)
  - [ ] Implement terminal detection for 4x4 boards (16 cells, k=3)
  - [ ] Ensure win conditions work with different k-in-row values
  - [ ] Handle board size routing based on GameState.config.boardSize
  - [ ] Test terminal detection with various board configurations
  - [ ] Validate performance across different board sizes

- [ ] **Task 5: Integration with Existing Engine Methods** (AC: 1)
  - [ ] Integrate isTerminal with existing kInRow implementation
  - [ ] Ensure winner method uses established win detection patterns
  - [ ] Maintain consistency with existing Engine interface contracts
  - [ ] Update applyMove to use terminal detection for state transitions
  - [ ] Preserve backward compatibility with existing engine functionality
  - [ ] Verify integration with legalMoves terminal state handling

- [ ] **Task 6: Comprehensive Unit Test Suite** (AC: 3)
  - [ ] Test isTerminal with won games (single and multiple winning lines)
  - [ ] Test isTerminal with draw games (board full, no winner)
  - [ ] Test isTerminal with ongoing games (moves remaining)
  - [ ] Test winner with various winning scenarios (rows, columns, diagonals)
  - [ ] Test winner returns null for draw games
  - [ ] Test both 3x3 and 4x4 board terminal detection
  - [ ] Test edge cases: simultaneous wins, early termination
  - [ ] Achieve 100% coverage on terminal detection logic
